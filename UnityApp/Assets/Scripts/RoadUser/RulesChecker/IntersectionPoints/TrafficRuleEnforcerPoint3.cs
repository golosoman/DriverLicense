...
public class TrafficRuleEnforcerPoint3 : TrafficRuleEnforcer
{
    public override bool CheckObstacleOnRight(GameObject roadUserDataObject, RoadUserData roadUserData)
    {
        ...
        foreach (KeyValuePair<RoadUserData, GameObject> roadUser in roadUsers[TagObjectNamesTypes.CAR])
        {
            ...
            {
                if (roadUserData.SidePosition == SideDirectionTypes.WEST && (roadUserData.MovementDirection == DirectionMovementTypes.FORWARD || 
                ...
                if (roadUserData.SidePosition == SideDirectionTypes.NORTH && (roadUserData.MovementDirection == DirectionMovementTypes.LEFT || 
                    roadUserData.MovementDirection == DirectionMovementTypes.BACKWARD))
                    { hasObstacleOnRight = true; break; }
                
                if (roadUserData.SidePosition == SideDirectionTypes.EAST && roadUserData.MovementDirection == DirectionMovementTypes.BACKWARD)
                    { hasObstacleOnRight = true; break; }
            }
        }
